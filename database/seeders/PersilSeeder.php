...
class PersilSeeder extends Seeder
{
    ...
    {
        // Ambil semua warga_id dari tabel warga
        $wargaIds = DB::table('warga')->pluck('warga_id')->toArray();

        // Jenis penggunaan (langsung string, sesuai persiladmin.sql)
        $penggunaanList = [
            'Pertanian', 'Perumahan', 'Komersial', 'Industri', 'Perkantoran',
            'Pendidikan', 'Kesehatan', 'Peribadatan', 'Lahan Kosong',
        ];
        ...
        for ($i = 1; $i <= 20; $i++) { // Buat 20 data saja dulu
            $wargaId    = ! empty($wargaIds) ? $wargaIds[array_rand($wargaIds)] : null;
            ...
            $data[] = [
                ...
                'pemilik_warga_id' => $wargaId, // Foreign key ke warga table
                'luas_m2'          => $luas,
                'penggunaan'       => $penggunaan, // String penggunaan

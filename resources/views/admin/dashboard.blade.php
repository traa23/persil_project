...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-white rounded-lg shadow-md p-8 hover:shadow-lg transition">
        <div class="flex items-center">
            ...
            <div>
                <p class="text-gray-600 text-sm font-medium">Total Guest User</p>
                <p class="text-4xl font-bold text-gray-800 mt-1">{{ $totalGuest }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-8 hover:shadow-lg transition">
        <div class="space-y-4">
            <a href="{{ route('admin.persil.create') }}" class="block px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition font-semibold text-center">
                <i class="fas fa-plus mr-2"></i> Tambah Persil
            </a>
            <a href="{{ route('admin.guest.create') }}" class="block px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-semibold text-center">
                <i class="fas fa-user-plus mr-2"></i> Tambah Guest
            </a>
        </div>
    </div>
</div>
                ...
                <tbody>
                    @foreach ($recentPersil as $persil)
                        <tr class="border-b hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-base text-gray-700">{{ $persil->pemilik->name }}</td>
